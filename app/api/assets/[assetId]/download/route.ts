...
export async function GET(_: Request, { params }: { params: Promise<{ assetId: string }> }) {
  ...
  const { data: asset } = await supabase
    .from("assets")
    .select("id, file_url, preview_url, generation_id, generations!inner(project_id, projects!inner(user_id))")
    .eq("id", assetId)
  ...
  if (!plan.fullResolutionExport) await trackEvent({ userId: user.id, projectId: (asset.generations as any).project_id, eventType: "paywall_viewed", metadata: { location: "asset_download" } });

  await trackEvent({ userId: user.id, projectId: (asset.generations as any).project_id, eventType: "asset_downloaded", metadata: { full: plan.fullResolutionExport } });
